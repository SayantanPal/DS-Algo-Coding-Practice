...
* Use [num & (1 << bit_position_from_right)] to CHECK if that particular bit_position_from_right contains 1 (SET BIT) or not. 
    * if(num & (1 << bit_position_from_right) != 0), that means bit_position_from_right contains '1' in binary representation of decimal no. num
      ...
      * OR Use while(n != 0) (n = n & n-1) // count - remove the lowest SET bit one-by-one [Brian Kernighan's Algo]
      * for all bit pos from i = 0(left) to 31(till right,say,for int datatype), if at ith pos the bit is set,
        then increment countOfSetBit or countOfOne ie if(n & (1 << i)) != 0, countOfSetBit++;
    * Even/Odd depends only on the last rightmost LSB bit-
      For even no, last rightmost LSB bit is 0 whereas for odd it is 1,
      ie. if (n & (1 << 0) != 0 ){ //OR if(n & 1 != 0) // means last bit is 1, then n & (1 << 0) equals 1 i.e., the no is odd
          // 'n' is Odd No.
      }
    * Reason: 
...
* In any number, 1 at MSB position is always greater and powerful than any combination of 1's except MSB position that means when MSB position unset to 0 
  ie single higher power of 2 contribution is even greater than total combined (summed) contribution of all possible lower powers of 2
  * Eg: 10 is always greater than 1, 100 is always greater than 011 ( which means also greater than 001, 010, 011), 1000 is always greater than 0111, 10000 is always greater than 01111
...
* BOTH A & 0 and A ^ A always leads to 0
* BOTH A & 1 and A | 0 leads to A
* A | 1 always leads to 1.
* Unpaired SET bit ie 1 only gives XOR of all bits as 1; Paired or unpaired UNSET bits does not matter to give output bit as 1
* Mask of 1 bit(SET bit) for a number can be generated by (Integer.highestOneBit(A) << 1) - 1. Eg: For 1010, Integer.highestOneBit(A) becomes 1000 and << 1 makes it 10000 and (Integer.highestOneBit(A) << 1) - 1 creates the mask 01111 which is essentially 1111
  * you can do A & Mask of Ones because logical '&' with Mask of 1 preserves bits. ie A & (Mask of A) returns A
  * If you want to toggle bits of A then ~A will have leading 0's converted to 1's and that can be trimmed off ~A & (Mask of 1's from MSB of A) = ~A & ((Integer.highestSetBit(A) << 1) - 1) ie ~A & (Mask of A) returns toggled bits of A
* A & B is 0 only when none of their bit positions are both SET ie every bit position has (0,1) or (1,0) or (0,0) BUT not (1, 1)
* To have XOR/OR of two numbers EQUAL to SUM of two numbers, the bits at any position should not be together SET ie NO OVERLAPPING SET bits, either they are 0,1 or 1,0 or 0,0
    * when A & B == 0, then output of XOR and OR becomes same as sum(addition)
    * The first unset bit larger than a no. A, say Y is always the bit just above A's highest set bit or MSB which comes from int Y = Integer.highestOneBit(A) << 1;
...
* if no of operations allowed is [a. multipled by 2 along b. +1 operation] for 0 -> n  or [a. divide by 2 along b. +1 operation] for n -> 0,
  ❯ formula is : Total cost/steps = (count no. of bits) for the *2 operations + (count no. of set bits) for the +1 operations
