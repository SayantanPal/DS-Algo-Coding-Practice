...
public class MainDriverExecSrvDemoCallableCyclicBarrier2 {
    public static void main(String[] args) {
            ...
            for (int j = 0; j < currentBatchSize; j++) {
                ...
                result.add(executorService.submit(() -> {
                    try {
                        ...
                        try {
                            if((totalTasks - finalI + 1) == 1)
                                cyclicBarrier.await(); // await is never block call - it never blocks parallel main thread exec. it just ensures all tasks to reach the barrier point and finishes their task, even parallely with main thread execution
        ...
        try {
            System.out.println("Factorial of 1 is: " + result.get(1).get());
