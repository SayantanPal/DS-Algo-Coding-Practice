...
* A | 1 always leads to 1. So, you can do A & Mask of Ones because logical & with Mask of 1 preserves bits. This can be generated by (Integer.highestOneBit(A) << 1) - 1. Eg: For 1010, Integer.highestOneBit(A) becomes 1000 and << 1 makes it 10000 and (Integer.highestOneBit(A) << 1) - 1 creates the mask 01111
  * If you want to toggle bits of A then ~A will have leading 0's converted to 1's and that can be trimmed off ~A & (Mask of 1's from MSB of A) = ~A & ((Integer.highestSetBit(A) << 1) - 1)
