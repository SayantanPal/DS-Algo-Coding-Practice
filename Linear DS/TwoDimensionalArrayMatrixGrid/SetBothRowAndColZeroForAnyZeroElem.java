...
public class SetBothRowAndColZeroForAnyZeroElem {
    ...
    public void setZeroes_v2(int[][] matrix) {
        ...
        // using 1st row and 1st col as visited array
        ...
        for(int i = 1; i < n; i++){
            for(int j = 1; j < m; j++){
                if(matrix[i][j] == 0){
                    matrix[i][0] = 0; // mark corresponding row in 1st col as zero
                    matrix[0][j] = 0; // mark corresponding col in 1st row as zero
                }
            }
        }
        ...
        for(int i = 1; i < n; i++){
            for(int j = 1; j < m; j++){
                if(matrix[i][0] == 0 || matrix[0][j] == 0){ // refer visited matrix to set corresponding cells to zero
                    matrix[i][j] = 0;
                }
            }
        }
